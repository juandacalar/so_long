...
{
    game->wall_img = mlx_xpm_file_to_image(game->mlx, "path_to_wall_texture.xpm", &game->tile_width, &game->tile_height);
    if (!game->wall_img)
    {
        ft_putstr_fd("Error: Failed to load wall texture.\n", 2);
        exit(EXIT_FAILURE);
    }
    game->player_img = mlx_xpm_file_to_image(game->mlx, "path_to_player_texture.xpm", &game->tile_width, &game->tile_height);
    if (!game->player_img)
    {
        ft_putstr_fd("Error: Failed to load player texture.\n", 2);
        exit(EXIT_FAILURE);
    }
    game->exit_img = mlx_xpm_file_to_image(game->mlx, "path_to_exit_texture.xpm", &game->tile_width, &game->tile_height);
    if (!game->exit_img)
    {
        ft_putstr_fd("Error: Failed to load exit texture.\n", 2);
        exit(EXIT_FAILURE);
    }
    game->collectible_img = mlx_xpm_file_to_image(game->mlx, "path_to_collectible_texture.xpm", &game->tile_width, &game->tile_height);
...
void draw_map(t_game *game)
{
    ...
    while (++y < game->map_height)
    {
        ...
        while (++x < game->map_width)
        {
            ...
            if (current_img)
                mlx_put_image_to_window(game->mlx, game->win, current_img, x * game->tile_width, y * game->tile_height);
        }
    }
}
